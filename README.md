...
**Status: Phase 3B complete — methodology validated against real data, no rankings published yet**

Phase 0 (data acquisition, `v0.1.0`) and Phase 1 (cleaning and reconciliation) are done — see `data/processed/merged_provincial_indicators.csv` and [notebooks/02_cleaning.ipynb](notebooks/02_cleaning.ipynb). Phase 2 (NPI methodology design — `docs/phase2_*.md`) is done. Phase 3 (`src/features/`, `src/scoring/`) is implemented, dry-run verified (`docs/phase3_dry_run.md`), and now validated end-to-end: outcome correlation (Spearman r = 0.72 overall, 0.63 for the 34 fully-covered provinces), weight sensitivity (robust across ±20%), leave-one-indicator-out, and PCA stability all computed against real data in [docs/phase3_validation_results.md](docs/phase3_validation_results.md). **No province rankings, percentiles, or priority tiers have been computed or published** — that is a deliberately separate, not-yet-authorized step. See [docs/known_limitations.md](docs/known_limitations.md) and [docs/province_reconciliation.md](docs/province_reconciliation.md) for structural limitations.
...
- **Phase 3 (complete through validation):** `src/features/` and `src/scoring/` built, dry-run verified (`docs/phase3_dry_run.md`), missing-data policy resolved (`docs/phase3_missing_data_decision.md`), and validated against real data (`docs/phase3_validation_results.md`) — outcome correlation, weight sensitivity, leave-one-indicator-out, PCA stability. No rankings, percentiles, or priority tiers exist yet — computing and publishing those is a separate, not-yet-authorized step.
- **Phase 4+ (not started):** validation execution, geospatial analysis, the dashboard, and policy insights.
